...
//  All sections handle API failure gracefully (no spinners).
// ============================================================
(function () {
  'use strict';
  ...
  // ── Global helper exposed for file input previews ──────────
  ...
  // ── Core API wrapper — ALWAYS catches network errors ───────
  async function api(method, ...args) {
    ...
    try {
      ...
      console.warn('[admin] API error — ' + method + ':', e.message);
      return null;
    }
  }
  ...
  function badgeClass(s) {
    const m = {
      confirmed:'badge-confirmed', completed:'badge-confirmed', paid:'badge-confirmed',
      present:'badge-confirmed',   active:'badge-confirmed',    converted:'badge-confirmed',
      pending:'badge-pending',     processing:'badge-pending',  contacted:'badge-pending',
      qualified:'badge-pending',   partial:'badge-pending',
      inquiry:'badge-new',         new:'badge-new',             late:'badge-new',
      cancelled:'badge-cancelled', lost:'badge-cancelled',      suspended:'badge-cancelled',
      absent:'badge-cancelled',    overdue:'badge-cancelled'
    };
  ...
  function fileField(inputId, previewId, label, urlFieldId) {
    ...
    return `<div class="adm-field">
      ...
      <div class="adm-upload-zone" onclick="document.getElementById('${inputId}').click()">
        <i class="fas fa-cloud-upload-alt"></i><span>Glisser / cliquer</span>
        <input type="file" id="${inputId}" accept="image/*" style="display:none"
          onchange="previewFile(this,'${previewId}','${ufId}')"/>
      </div>
      <input class="adm-input" id="${ufId}" placeholder="…ou coller une URL" style="margin-top:.4rem;"
        oninput="(function(i){const p=document.getElementById('${previewId}');if(p&&i.value){p.src=i.value;p.style.display='block';}})(this)"/>
  ...
  async function resolveUpload(inputId) {
    ...
    try {
      const j = await window.HorizonAPI.upload(form);
      return j?.url || null;
    } catch { return null; }
  }
  async function resolveImgUrl(inputId, urlFieldId) {
    const uploaded = await resolveUpload(inputId);
    return uploaded || getVal(urlFieldId || inputId+'_url');
  }
  ...
  function renderPagination(container, pagination, current, onPage) {
    if (!pagination?.total) { container.innerHTML=''; return; }
  ...
  async function loadSection(sec) {
    ...
    const map = { dashboard, bookings, clients, leads, journal, offers, gallery, videos, staff, attendance, reminders, settings };
    try {
      if (map[sec]) await map[sec](c);
      else c.innerHTML = '<div class="no-data">Section à venir</div>';
    } catch (e) {
      console.error('[admin] loadSection error:', e);
      c.innerHTML = `<div class="adm-error-box"><i class="fas fa-exclamation-triangle"></i> Erreur de chargement.<br/><small>${esc(e.message)}</small><br/><button class="adm-btn" style="margin-top:.75rem;" onclick="loadSection('${sec}')">Réessayer</button></div>`;
    }
  }
  ...
  async function refreshClients() {
    ...
    if (!res) {
      wrap.innerHTML = emptyState('API non disponible — vérifiez le backend','Réessayer','loadSection(\'clients\')');
    ...
    wrap.innerHTML = `<div class="adm-tbl-wrap"><table class="adm-tbl">
      ...
      <tbody>${list.map(cl=>{
        ...
        return `<tr data-batch-id="${id}" data-phone="${esc(cl.phone)}" data-name="${esc(cl.name)}">
          <td><input type="checkbox" class="adm-row-cb" data-section="clients" data-id="${id}"
            onchange="batchToggleRow('clients','${id}',this)"/></td>
          <td>${cl.photo_url
          ...
          <td>${esc(cl.phone)}${waOk?' <span class="adm-wa-badge"><i class="fab fa-whatsapp"></i></span>':''}</td>
          <td>${esc(cl.email||'—')}</td>
          <td>${esc(cl.wilaya||'—')}</td>
          <td>${esc(cl.assigned_staff_name||'—')}</td>
          <td>
            <button class="adm-act-btn" onclick="openClientModal('${id}')"><i class="fas fa-edit"></i></button>
            ${waOk?`<button class="adm-act-btn wa" onclick="waContact('${esc(cl.phone)}','${esc(cl.name)}')"><i class="fab fa-whatsapp"></i></button>`:''}
            <button class="adm-act-btn del" onclick="deleteClient('${id}')"><i class="fas fa-trash"></i></button>
          </td>
        </tr>`;
      }).join('')}</tbody>
    </table></div>`;
  ...
  window.saveClient = async function (uuid) {
    ...
    if (res!==null) { removeModal(); await refreshClients(); toast('Client enregistré','ok'); }
    else toast('Erreur API — vérifiez le backend','err');
  };
  ...
  async function bookings(c) {
    c.innerHTML = `
      <div class="adm-card">
        <div class="adm-card-head">
          ...
          <div class="adm-card-actions">
            <select class="adm-select" id="bkTypeFilter">
            ...
            <select class="adm-select" id="bkStatusFilter">
    ...
    document.getElementById('bkTypeFilter')?.addEventListener('change', async e => {
      FILTERS.bookings.type=e.target.value; PAGE.bookings=1; await refreshBookings();
    });
    document.getElementById('bkStatusFilter')?.addEventListener('change', async e => {
      FILTERS.bookings.status=e.target.value; PAGE.bookings=1; await refreshBookings();
    });
  ...
  async function refreshBookings() {
    ...
    wrap.innerHTML = `<div class="adm-tbl-wrap"><table class="adm-tbl">
      ...
      <tbody>${list.map(b=>{
        ...
        return `<tr data-batch-id="${id}" data-phone="${esc(b.client_phone||'')}" data-name="${esc(b.client_name||'')}">
          <td><input type="checkbox" class="adm-row-cb" data-section="bookings" data-id="${id}"
            onchange="batchToggleRow('bookings','${id}',this)"/></td>
          <td style="font-family:monospace;font-size:.72rem;">${String(id).slice(0,8)}</td>
          <td class="nm">${esc(b.client_name||'—')}</td>
          ...
          <td><span class="adm-badge ${badgeClass(b.status)}">${b.status||''}</span>
            ${b.payment_status?`<span class="adm-badge ${badgeClass(b.payment_status)}" style="font-size:.65rem;margin-left:.25rem;">${b.payment_status}</span>`:''}
          </td>
          <td>
            <button class="adm-act-btn" onclick="openBookingModal('${id}')"><i class="fas fa-edit"></i></button>
            <button class="adm-act-btn del" onclick="deleteBooking('${id}')"><i class="fas fa-trash"></i></button>
          </td>
        </tr>`;
      }).join('')}</tbody>
    </table></div>`;
  ...
  function openBookingModal(uuid) {
    const modal = buildModal(uuid?'Modifier réservation':'Nouvelle réservation', `
      <div class="adm-field"><label>Client *</label>
        <select class="adm-input" id="bkClientId"><option value="">Chargement…</option></select></div>
      <div class="adm-field"><label>Service *</label>
        <select class="adm-input" id="bkServiceType">
          <option value="omra">Omra</option><option value="hajj">Hajj</option>
          <option value="visa">Visa</option><option value="flight">Vol</option>
          <option value="hotel">Hôtel</option><option value="package">Forfait</option>
        </select></div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:.75rem;">
        <div class="adm-field"><label>Montant total (DZD)</label>
          <input class="adm-input" type="number" id="bkTotalAmount" min="0" step="1000"/></div>
      ...
      <div class="adm-field"><label>Statut paiement</label>
        <select class="adm-input" id="bkPayStat">
          <option value="pending">En attente</option><option value="partial">Partiel</option>
          <option value="paid">Payé</option><option value="overdue">En retard</option>
        </select></div>
    ...
    api('getClients',{limit:1000}).then(res => {
      ...
      sel.innerHTML = '<option value="">Sélectionner un client…</option>' +
        list.map(cl=>`<option value="${cl.id||cl.uuid}">${esc(cl.name)} — ${esc(cl.phone)}</option>`).join('');
    });
    ...
    if (uuid) {
      api('getBooking', uuid).then(b => {
        if (!b) return;
        setVal('bkClientId',    b.client_id);
        setVal('bkServiceType', b.type);
        setVal('bkStat',        b.status);
        setVal('bkDepart',      b.travel_date?.split('T')[0]);
        setVal('bkRetour',      b.return_date?.split('T')[0]);
        setVal('bkTotalAmount', b.total_amount);
        setVal('bkAmountPaid',  b.amount_paid||0);
        setVal('bkPayStat',     b.payment_status||'pending');
        setVal('bkNotes',       b.notes);
      });
    }
  }
  ...
  window.saveBooking = async function (uuid) {
    ...
    const data = {
      client_id:      clientId,
      type:           getVal('bkServiceType'),   // correct ID: bkServiceType
    ...
    const res = uuid ? await api('updateBooking',uuid,data) : await api('submitBooking',data);
    if (res!==null) { removeModal(); await refreshBookings(); toast('Réservation enregistrée','ok'); }
  ...
  async function refreshLeads() {
    ...
    if (!wrap) return;
    wrap.innerHTML = '<div style="text-align:center;padding:2rem;color:rgba(255,255,255,.3);"><i class="fas fa-spinner fa-spin"></i></div>';
    const res = await api('getLeads',{limit:PER.leads, offset:(PAGE.leads-1)*PER.leads, ...FILTERS.leads});
    ...
    wrap.innerHTML = `<div class="adm-tbl-wrap"><table class="adm-tbl">
      ...
      <tbody>${list.map(l=>`<tr data-batch-id="${l.id}" data-phone="${esc(l.phone)}" data-name="${esc(l.name)}">
        ...
        <td style="display:flex;gap:.25rem;flex-wrap:wrap;">
          ...
          <button class="adm-act-btn" onclick="updateLeadStatus('${l.id}','contacted')" title="Contacté"><i class="fas fa-phone"></i></button>
          <button class="adm-act-btn" style="color:#f87171" onclick="updateLeadStatus('${l.id}','lost')" title="Perdu"><i class="fas fa-times"></i></button>
        </td>
      </tr>`).join('')}</tbody>
    </table></div>`;
      ...
      <div class="adm-card">
        <div class="adm-card-head">
          ...
          <div class="adm-card-actions">
            ...
            <button class="adm-btn success" onclick="openTransactionModal()"><i class="fas fa-plus"></i> Nouvelle transaction</button>
          </div>
        </div>
        ${txs.length
          ...
          : emptyState('Aucune transaction','Ajouter une transaction','openTransactionModal()')}
      </div>`;
  }
  window.exportJournalCSV = async function () {
    const res = await api('getTransactions',{limit:1000});
    const txs = Array.isArray(res)?res:(res?.data||[]);
    if (!txs.length) { toast('Aucune transaction à exporter',''); return; }
  ...
  window.openTransactionModal = function () {
    const modal = buildModal('Nouvelle transaction',`
      ...
      <div class="adm-field"><label>Méthode</label>
      ...
      <div class="adm-field"><label>Référence</label><input class="adm-input" id="txRef" placeholder="N° reçu…"/></div>
      <div class="adm-field"><label>Description</label><textarea class="adm-input adm-textarea" id="txDesc"></textarea></div>
    `,'saveTx()');
    document.body.appendChild(modal);
  };
    ...
    c.innerHTML = `
      <div class="adm-card">
        <div class="adm-card-head">
          ...
          <button class="adm-btn success" onclick="openStaffModal(null)"><i class="fas fa-plus"></i> Ajouter</button>
        </div>
        ${!list
          ? emptyState('Backend non disponible','Réessayer',"loadSection('staff')")
          : !list.length
            ...
            : `<div class="adm-tbl-wrap"><table class="adm-tbl">
                ...
                <tbody>${list.map(s=>{
                  ...
                  return `<tr>
                    ...
                    <td>
                      <button class="adm-act-btn" onclick="openStaffModal('${id}')"><i class="fas fa-edit"></i></button>
                      <button class="adm-act-btn del" onclick="deleteStaff('${id}')"><i class="fas fa-trash"></i></button>
                    </td>
                  </tr>`;
                }).join('')}</tbody>
              </table></div>`}
  ...
  function openStaffModal(uuid) {
    ...
    if (isEdit) {
      api('getStaff').then(list=>{
        const s=(list||[]).find(x=>(x.uuid||x.id)===uuid); if(!s)return;
  ...
  window.saveStaff = async function (uuid) {
    ...
    if (photoUrl) data.photo_url = photoUrl;
    if (!data.first_name||!data.last_name||!data.email) { toast('Prénom, nom et email requis','err'); return; }
    if (!uuid&&(!data.password||data.password.length<8)) { toast('Mot de passe minimum 8 caractères','err'); return; }
  ...
  async function gallery(c) {
    ...
    c.innerHTML = contentGrid('gallery', list, 'Galerie', g=>({
      title: g.data.caption||'', sub:g.data.alt||'',
  ...
  function contentGrid(type, list, label, mapper) {
    const tl = {offer:'offre',gallery:'photo',video:'vidéo'}[type]||type;
    return `
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;">
        <h2 style="color:rgba(255,255,255,.7);font-family:'Playfair Display',serif;font-size:1.1rem;">${label}</h2>
        <button class="adm-btn success" onclick="openModal('${type}',null)"><i class="fas fa-plus"></i> Ajouter</button>
      </div>
      ${!list.length ? emptyState('Aucun contenu','Ajouter '+tl,`openModal('${type}',null)`) :
        `<div class="adm-grid">
          ${list.map(item=>{
            const m=mapper(item), id=item.uuid||item.id;
            return `<div class="adm-content-card">
              ...
              <div class="adm-content-card-body">
                ...
                <div class="adm-content-card-actions">
                  <button class="adm-btn" onclick="openModal('${type}','${id}')">Modifier</button>
                  <button class="adm-btn" onclick="toggleActive('${type}','${id}')">${item.is_active?'Masquer':'Afficher'}</button>
                  <button class="adm-btn danger" onclick="deleteItem('${type}','${id}')"><i class="fas fa-trash"></i></button>
                </div>
              </div>
            </div>`;
          }).join('')}
          <div class="adm-add-card" onclick="openModal('${type}',null)">
            <i class="fas fa-plus"></i><span>Ajouter ${tl}</span>
          </div>
        </div>`}`;
  }

  // ── Content modal (static one from HTML) ───────────────────
  function openModal(type, id) {
    modalMode=type; modalEditId=id;
    ...
    if (type==='offer') {
      b.innerHTML=`
        ...
        ${fileField('mf_imgFile','mf_imgPrev','Image de l\'offre','mf_img')}`;
      if (id) api('adminGetContent','offer').then(list=>{
        ...
        if(o.data.image_url){const p=document.getElementById('mf_imgPrev');if(p){p.src=o.data.image_url;p.style.display='block';}}
      });
    } else if (type==='gallery') {
      b.innerHTML=`
        ${fileField('mf_imgFile','mf_imgPrev','Photo','mf_img')}
      ...
      if (id) api('adminGetContent','gallery').then(list=>{
        ...
        if(g.data.image_url){const p=document.getElementById('mf_imgPrev');if(p){p.src=g.data.image_url;p.style.display='block';}}
      });
    } else if (type==='video') {
      b.innerHTML=`
        ...
        ${fileField('mf_thumbFile','mf_thumbPrev','Miniature','mf_thumb')}`;
      if (id) api('adminGetContent','video').then(list=>{
        ...
        if(v.data.thumbnail_url){const p=document.getElementById('mf_thumbPrev');if(p){p.src=v.data.thumbnail_url;p.style.display='block';}}
      });
    }
  }
  ...
  async function saveModal() {
    ...
    if (type==='offer') {
      ...
      data={title:{fr:getVal('mf_t_fr'),ar:getVal('mf_t_ar'),en:getVal('mf_t_en')},
            description:{fr:getVal('mf_d_fr'),ar:getVal('mf_d_ar')},price,image_url:imgUrl};
    ...
    c.innerHTML = `
      <div class="adm-card">
        ...
        <div style="padding:1.5rem;">
          ...
          ${saved.logo_url?`<div class="adm-field"><label>Logo actuel</label>
            <img src="${esc(saved.logo_url)}" style="max-height:60px;border-radius:6px;display:block;margin-top:.4rem;"/></div>`:''}
      ...
      <div class="adm-card" style="margin-top:1rem;">
        ...
        <div style="padding:1.5rem;display:flex;align-items:center;gap:1rem;">
          <span style="color:rgba(255,255,255,.5);">Mode actuel :</span>
  ...
  window.saveBranding = async function () {
    ...
    api('updateAgency',branding).then(res=>{
      toast(res!==null?'Branding sauvegardé sur le serveur ✓':'Branding appliqué localement (API indisponible)','ok');
    });
  };
  window.doLogout = async function () {
    await api('logout'); adminOk=false; closeAdmin(); toast('Déconnecté','ok');
  };
  ...
  function initInlineEdit() {
    ...
    panel.addEventListener('dblclick', async function(e){
      ...
      const save=async()=>{
        ...
        const map={client:()=>api('updateClient',id,{[field]:nv}),booking:()=>api('updateBooking',id,{[field]:nv})};
        try{if(map[entity])await map[entity]();td.textContent=nv;td.dataset.orig=nv;toast('Mis à jour','ok');}
        catch(err){td.textContent=orig;toast('Erreur: '+err.message,'err');}
      };
      input.addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();save();}if(e.key==='Escape'){td.textContent=orig;}});
  ...
  _css.textContent=`
    ...
    td[data-field]{cursor:text;}td[data-field]:hover{background:rgba(201,162,39,.05);}
    ...
    body.theme-light .adm-tbl thead th{background:#f0f4fa;color:#4a5568;}
  ...
  function init() {
    ...
    // Wire static modal buttons from HTML
