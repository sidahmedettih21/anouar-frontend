(function() {
var lang = localStorage.getItem("aes_lang") || "en";
(function() {
(function() {
(function() {
...
function playHeroVideo() {
  ...
  const v = vids[0];
  if (!v) return;
  document.getElementById('heroVideoPoster').innerHTML = `<iframe src="${v.embedUrl}" style="width:267px;height:476px;border:none;" allowfullscreen></iframe>`;
}
function playWatchVideo() {
  const vids = window.DEF_VIDEOS || [];
  const v = vids[1] || vids[0];
  if (!v) return;
  document.getElementById('watchVideoWrap').innerHTML = `<iframe src="${v.embedUrl}" style="width:267px;height:476px;border:none;" allowfullscreen></iframe>`;
}
    ...
    grid.innerHTML = offers.map(o => {
      ...
      return `<div class="offer-card reveal">
        ...
        <div class="offer-content">
          ...
          <div class="offer-price">${Number(data.price).toLocaleString()} DZD<span> / ${TR[lang]?.per_person||'per person'}</span></div>
          <button class="offer-btn" onclick="bookOffer('${esc(title)}')">${TR[lang]?.book_now||'Book Now'} <i class="fas fa-arrow-right"></i></button>
        </div>
      </div>`;
    }).join('');
    observeAll();
...
function clearErr(el) { el.classList.remove('error'); el.parentNode.querySelector('.f-err')?.remove(); }
...
function setLang(l) {
  ...
  document.querySelectorAll('[data-i18n]').forEach(el => {
    ...
    if (TR[l] && TR[l][k]) {
      if (['INPUT', 'TEXTAREA'].includes(el.tagName)) el.placeholder = TR[l][k];
      else el.textContent = TR[l][k];
    }
  });
  document.querySelectorAll('[data-i18n-ph]').forEach(el => {
    const k = el.dataset.i18nPh;
    if (TR[l] && TR[l][k]) el.placeholder = TR[l][k];
  });
